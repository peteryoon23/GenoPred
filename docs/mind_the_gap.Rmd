...
score_file <- score_file[, c('SNP','A1','A2', names(score_file)[grepl('SBayesRC', names(score_file))]), with =F]
...
for(prop_i in c(0.5, 0.25, 0.05)){
  ...
  names(score_i) <- gsub('SCORE_SBayesRC',paste0('SCORE_SBayesRC_rsq_', prop_i), names(score_i))
  score_all <- cbind(score_all, score_i[, grepl('SBayesRC', names(score_i)), with = F])
}
